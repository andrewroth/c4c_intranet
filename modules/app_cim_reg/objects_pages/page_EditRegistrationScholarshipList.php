 ...
 */ 
/**
 * class FormProcessor_EditRegistrationScholarshipList 
 * <pre> 
 * A (sub-)page used to edit scholarships for a particular registration record.
 * </pre>
 * @author Russ Martin
 * Date:   10 Jul 2007
 */
 // RAD Tools: AdminBox Page
class  FormProcessor_EditRegistrationScholarshipList extends PageDisplay_FormProcessor_AdminBox {
    ...
    //                             to form_value != '' 
    const FORM_FIELDS = 'scholarship_sourceDesc|T|,scholarship_sourceAcct|T|,scholarship_amount|N|,registration_id|T|<skip>,form_name|T|<skip>';
    
    /** The list of field types to be displayed in the form */
    // NOTE: if a field isn't displayed, put a '-' for it's entry.
    const FORM_FIELD_TYPES = 'textbox,textbox,textbox,-,hidden';
    
    /** The list of fields to be displayed in the data list */
    const DISPLAY_FIELDS = 'scholarship_sourceDesc,scholarship_sourceAcct,scholarship_amount';
    
    /** The Querystring Field for which entry is currently being edited ... */
    const MULTILINGUAL_PAGE_KEY = 'FormProcessor_EditRegistrationScholarshipList';
...
/* no List Init Variable defined for this DAObj */
	 ...
	 * @param $scholarship_id [STRING] The init data for the dataManager obj
	 * @param $registration_id [INTEGER] The foreign key data for the data Manager
	 * @return [void]
	 */
    function __construct($pathModuleRoot, $viewer, $formAction, $sortBy, $scholarship_id='', $registration_id)
    {
        // NOTE: be sure to call the parent constructor before trying to 
        //       use the ->formXXX arrays...
        $fieldList = FormProcessor_EditRegistrationScholarshipList::FORM_FIELDS;
        $fieldTypes = FormProcessor_EditRegistrationScholarshipList::FORM_FIELD_TYPES;
        $displayFields = FormProcessor_EditRegistrationScholarshipList::DISPLAY_FIELDS;
        ...
        if ($scholarship_id != '') 
        {
        		...
        		$this->dataManager = new RowManager_ScholarshipAssignmentManager($scholarship_id);
     		}
     		else
     		{
	     		$this->dataManager = new RowManager_ScholarshipAssignmentManager();
     		}
 ...
 //       $this->dataManager = new RowManager_ScholarshipAssignmentManager();	// $scholarship_id );
        ...
        $this->dataManager->setRegID($registration_id);
        
        if ($this->scholarship_id != '') {
        		$this->dataManager->loadByScholarshipID($this->scholarship_id);
     	  }
...
///        }
    ...
    function processData() 
    {
        ...
        // on a successful update return scholarship_id to ''
        $this->scholarship_id='';

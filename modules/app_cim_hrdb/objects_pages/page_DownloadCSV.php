 ...
 * <pre> 
 * A generic report download page (for a single report type)
 * </pre>
 * @author Hobbe Smit
 * Date:   28 Jan 2008
 */
class  page_DownloadCSV extends PageDisplay {
...
//    const DISPLAY_FIELDS = '';
    ...
    const MULTILINGUAL_PAGE_KEY = 'page_DownloadCSV';
    ...
    /** @var [STRING] The report ID filter for the data to download. */
	protected $report_id;
	
    /** @var [STRING] The director ID filter for the data to download. */
	protected $director_id;	
					
	
	/** @var [INTEGER] The name of the created file */
   protected $file_name;
	 ...
	 * @param $campusID [STRING] Identifies the campus to filter data by
     * @param $file_name_list [STRING] CSV list of possible file names (1 per download type)
	 * @return [void]
	 */
    function __construct($pathModuleRoot, $viewer, $downloadType, $campusID = '', $campusStatusFilter = '', $file_name_list = '', $reportID = '' ) 
    {
        parent::__construct( );
        
        $this->pathModuleRoot = $pathModuleRoot;
        $this->viewer = $viewer; 
        $this->director_id = $this->getStaffIDfromViewerID();  
        
        // overwrites data in a file determined by $reportType and located at SITE_PATH_REPORTS
        $this->file_name = '';
        $this->file_name = $this->storeReportData($downloadType, $campusID, $campusStatusFilter, $file_name_list, $reportID, $this->director_id);
 ...
 */        
         ...
         $pageKey = page_DownloadCSV::MULTILINGUAL_PAGE_KEY;
    ...
    function getHTML() 
    {
        ...
        switch ($this->download_type)
        {
	        case  modulecim_hrdb::DOWNLOAD_SENIORITY_LIST:
	        		$downloadLinks[ 'DownloadSeniorityDataDump' ]  = $this->linkValues[ 'DownloadSeniorityDataDump' ];
	        		break;
	        case  modulecim_hrdb::DOWNLOAD_ACTIVITIES_LIST:
	        		$downloadLinks[ 'DownloadActivitiesDataDump' ]  = $this->linkValues[ 'DownloadActivitiesDataDump' ];
	        		break;	 
	        case  modulecim_hrdb::DOWNLOAD_CUSTOM_REPORT:
	        		$downloadLinks[ 'DownloadCustomReportDataDump' ]  = $this->linkValues[ 'DownloadCustomReportDataDump' ];
	        		break;	        		       		
	        default:
	        		break;
        	}
...
//        $this->template->set( 'disableHeading', true );
    ...
    protected function storeReportData($reportType, $campusID = '', $campus_status_filter = '', $file_names = '', $reportID = '', $directorID = '')
    {
...
//    	echo "FileData = ".$fileData; 
 		...
 		{
	 		case modulecim_hrdb::DOWNLOAD_SENIORITY_LIST:
				$fileName =  $fileNames[0];	//modulecim_hrdb::EVENT_DATA_FILE_NAME;
// 				$filePrefix = modulecim_hrdb::EVENT_DATA_FILE_NAME;
// 				$filePath = tempnam($fileDir, $filePrefix);
    			$fileData = $dataGetter->getStudentSeniorityCSV($campusID, $campus_status_filter, $directorID);		
	 			break;
	 		case modulecim_hrdb::DOWNLOAD_ACTIVITIES_LIST:
				$fileName =  $fileNames[0];
     			$fileData = $dataGetter->getStaffActivitiesCSV($directorID);		
	 			break;
	 		case modulecim_hrdb::DOWNLOAD_CUSTOM_REPORT:
	 			$reportTitle = $dataGetter->getCustomReportTitleAbbrev($reportID);
				$fileName =  $reportTitle.$fileNames[0];
     			$fileData = $dataGetter->getCustomReportCSV($reportID,$directorID);		
	 			break;	 			
	 		default:
	 			break;
 		}
			...
			//if (is_writable($filename)) {
			   ...
			   if (!$handle = fopen($fileDir.$fileName, 'w')) {	//$filePath, 'w')) {	//
	...
	// A simple function for returning the last file name

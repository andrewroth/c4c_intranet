 ...
 */ 
/**
 * class FormProcessor_EditStaffFormInstructions 
 * <pre> 
 * Use this (sub-)page to add/edit form instructions for a given HRDB form.
 * </pre>
 * @author CIM Team
 * Date:   05 Mar 2008
 */
 // RAD Tools: FormSingleEntry Page
class  FormProcessor_EditStaffFormInstructions extends PageDisplay_FormProcessor {
    ...
    //                             '<skip>' for this value. 
    const FORM_FIELDS = 'staffscheduleinstr_toptext|T|,staffscheduleinstr_bottomtext|T|,staffscheduletype_id|T|<skip>,form_name|T|<skip>';
    
    /** The list of field types to be displayed in the form */
    // NOTE: if a field isn't displayed, put a '-' for it's entry.
    const FORM_FIELD_TYPES = 'textarea,textarea,-,hidden';
    
    /** The Multilingual Page Key for this page */
    const MULTILINGUAL_PAGE_KEY = 'FormProcessor_EditStaffFormInstructions';
	...
	protected $scheduleinstr_id;

    /** @var [BOOLEAN] Whether or not page heading is disabled */
	protected $disableHeading;
	
    /** @var [BOOLEAN] Whether or not to create a new DB entry */
...
//  	   	echo 'submitted form values: <pre>'.print_r( $this->formValues,true).'</pre><br>';	 
       
        
        // store values in table manager object.
        $this->formValues[ 'staffscheduletype_id' ] = $this->staffscheduletype_id;
        $this->dataManager->loadFromArray( $this->formValues );
        
        // now update the DB with the values
        if ($this->createNewEntry == true) {		//(!$this->dataManager->isLoaded()) {
            $db_error_msg = $this->dataManager->createNewEntry(true);
            $this->setErrorMessage($db_error_msg);	// TODO: get this to work
        } else {
    ...
    function getHTML() 
    {
        ...
        $this->template->set( 'buttonText', $this->labels->getLabel('[UpdateForm2]') );
        
  		  $this->formValues['form_name'] = 'instructionsForm';
		...
		//$templateName = 'page_EditStaffFormInstructions.php';

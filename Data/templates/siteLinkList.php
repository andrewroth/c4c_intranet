...
//    $entryKey = $rowManagerXMLNodeName;
    ...
    foreach( $linksList as $desc=>$link )		//$dataList->$entryKey as $entry) {
	 {
		 echo '<li><a href="'.$link.'">'.$templateTools->getPageLabel('['.$desc.']').'</a></li>';
	 }
...
//    }
    ...
    if ( isset($linkValues['cont']) ) {
        $continueLabel = '<a href="'.$linkValues['cont'].'">'.$templateTools->getPageLabel('[Continue]').'</a>';
    }

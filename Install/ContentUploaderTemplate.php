...
# 16 Dec 04
#
# If we decide to add additional parameters to the QueryString we need to 
# not assume that the sessionid is the 1st element. So I changed the following:
...
# to :
my $originalQuerystringParameters = $ENV{"QUERY_STRING"};

@qstring=split(/&/, $originalQuerystringParameters );
foreach my $entry (@qstring) {

    @p1 = split(/=/,$entry);
    if ( $p1[0] eq "<? echo PerlFileUpload::DEF_QSPARAM_SID ?>" ) {
    
        $sessionid = $p1[1];
        $sessionid =~ s/[^a-zA-Z0-9]//g;  # sanitized as suggested by Terrence Johnson.
     } elsif ( $p1[0] eq "<? echo PerlFileUpload::DEF_QSPARAM_UPLOADER ?>" ) {
...
# 16 Dec 04
#
# Now tack on all those original Query String Parameters we collected earlier
# to the php_uploader so the processing page can have access to those values.
#
# removed:
...
my $url = $php_uploader . "?$originalQuerystringParameters";
